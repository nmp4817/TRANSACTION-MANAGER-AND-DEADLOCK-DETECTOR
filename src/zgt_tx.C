...
void *readtx(void *arg){
  ...
  if(tx!=NULL)				//Checking that TX is available in TX MGR that is abort or commit isn't called yet
 {
	  zgt_p(0);
 	  zgt_hlink* temp =  ZGT_Ht->findt (node->tid,1, node->obno);		//If TX itself holding a Lock
	  if((temp) != NULL)
	  {
	      tx->perform_readWrite(node->tid,node->obno,'S');			//Perform write directly
	      zgt_v(0);
	  }
	
	  else
	  { 
	      a = tx->set_lock(node->tid,1, node->obno, node->count,'S');	//If TX isn't holding lock
	      zgt_v(0);
	  }
	  if(a!=0)
	  {
		 cout<<"Error occure during aquring lock!";
	  }
  }
...
void *writetx(void *arg){ //do the operations for writing; similar to readTx
  ...
  if(tx!=NULL)				//Checking that TX is available in TX MGR that is abort or commit isn't called yet
  {
	  ...
	  if((temp) != NULL)
	  {
	      tx->perform_readWrite(node->tid,node->obno,'X');			//Perform write directly
	      zgt_v(0);
	  }
	  else
	  { 
	      a = tx->set_lock(node->tid,1, node->obno, node->count,'X');	//If TX isn't holding lock
	      zgt_v(0);
	  }
...
void *committx(void *arg)
{
 //remove the locks before committing
  ...
  open_logfile_for_append();
  fprintf(logfile, "T%d\t%c \tCommitTx\n", node->tid, node->Txtype);    // Write log record and close
  fflush(logfile);
...
{
 zgt_p(0);
  tx->free_locks();			//free all lock aquired by Tx. i.e. Removing entry from hash table
  tx->remove_tx();			//Remove TX from TX MGR.
  
  a = zgt_nwait(tx->semno);		// check number of Tx. waiting on this Tx
  for(int i=0; i<a;i++)
  {
	zgt_v(tx->semno);		//release all waiting TX.
  }
 zgt_v(0);
}
}
...
int zgt_tx::set_lock(long tid1, long sgno1, long obno1, int count, char lockmode1){
  ...
  if(this->status!=TR_ABORT || this->status!=TR_END)
{
  zgt_hlink* temp = ZGT_Ht->find(sgno1,obno1);				//Checking if any other TX. is holding lock

  if((temp) != NULL)							//If any other Tx is holding lock, if not then directly lock the object
  {
	this->status = TR_WAIT;						//Change status of waiting Tx. to TR_WAIT
	this->lockmode = lockmode1;					//Change Lockmode of waiting Tx.
	this->setTx_semno(temp->tid,temp->tid);				//Change semno of Tx. which is holding lock
	zgt_tx *tx = get_tx(temp->tid);					//get the pointer to object of Tx. which is holding lock
	wait_for *wf = new wait_for();
	int d = wf->deadlock();						//Checking Deadlock

	if(d == 0 && this->status == TR_ABORT)
	{
		free_locks();
		remove_tx();
		for(int i=0; i<zgt_nwait(this->semno);i++)
  		{
			zgt_v(this->semno);				//release all waiting TX.
  		}
	...
	zgt_p(tx->semno);						//call the zgt_p() on semaphore of Tx.which is holding lock
	zgt_p(0);
	int b = this->set_lock(tid1,sgno1,obno1,count,lockmode1);	//recursively call
	return b;
  }
	this->lockmode = ' ';
	this->status = TR_ACTIVE;					//change status of waiting tx. to TR_ACTIVE from TR_WAIT

  int a = ZGT_Ht->add(this,sgno1,obno1,lockmode1);			//actual locking : adding node to a hash table
  this->perform_readWrite(tid1,obno1,lockmode1);			// perform read write
  return a;
}
}
